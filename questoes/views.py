...
def filtrar(request):
    ...
    if form.is_valid():
        ...
        if data.get('modalidade'):
            query.add(Q(modalidade = data.get('modalidade')), Q.AND)
        if data.get('staff'):
            query.add(Q(staff = data.get('staff')), Q.AND)
        if data.get('ano'):
            query.add(Q(ano = data.get('ano')), Q.AND)
        if data.get('nivel'):
            query.add(Q(nivel = data.get('nivel')), Q.AND)
        if data.get('confederacao'):
            query.add(Q(confederacao = data.get('confederacao')), Q.AND)
